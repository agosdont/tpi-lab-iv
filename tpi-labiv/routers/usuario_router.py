...
from schemas.usuario_schema import UsuarioSchema, UsuarioAuth
from fastapi.responses import JSONResponse
...
def authenticate_user(users:dict, email: str, password: str)->UsuarioSchema:
    user = get_user(users, email)
    if not user:
        return False
    if not verify_password(password, user.password):
        return False
    user = UsuarioSchema.model_validate(user)
    return user
...
@usuario_router.get('/usuarios', tags=['Usuarios'], response_model=List[UsuarioSchema], status_code=200, dependencies=[Depends(JWTBearer())])
...
@usuario_router.get('/usuarios/{id}', tags=['Usuarios'], response_model=UsuarioSchema, dependencies=[Depends(JWTBearer())])
def get_usuario(id: int = Path(ge=1, le=2000)) -> UsuarioSchema:
    db = Session()
...
@usuario_router.get('/usuarios/', tags=['Usuarios'], response_model=List[UsuarioSchema], dependencies=[Depends(JWTBearer())])
def get_usuario_email(email: str = Query(min_length=5, max_length=35)) -> List[UsuarioSchema]:
    db = Session()
    result = UsuarioService(db).get_usuario_email(email)
    return JSONResponse(status_code=200, content=jsonable_encoder(result))


@usuario_router.post('/usuarios', tags=['Usuarios'], response_model=dict, status_code=201)
def create_usuario(usuario: UsuarioSchema) -> dict:
...
def update_usuario(id: int, usuario: UsuarioSchema)-> dict:
    usuario.password = get_password_hash(usuario.password.get_secret_value())
    db = Session()
    result = UsuarioService(db).get_usuario(id)
    if not result:
